package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1250:uint = 1;
      
      private static const const_922:uint = 2;
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_1977 == param3)
               {
                  return;
               }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(_loc4_)
            {
               ...
               {
                  if(_loc6_.callback == param2 && _loc6_.var_1977 == param3)
                  {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!this._disposed)
         {
            ...
            if(this._result == const_922)
            {
               if(this.var_560 != null)
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ": " + this.var_560.message,this.var_560.errorID,this.var_560);
               }
               else
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ". No error data available!",0,this.var_560);
               }
            }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         this._result = !!event.isDefaultPrevented() ? uint(const_1250) : uint(const_596);
      }
