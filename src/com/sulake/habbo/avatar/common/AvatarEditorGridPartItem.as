package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_771:IFigurePartSet;
      ...
      private var var_429:IAvatarRenderManager;
      
      private var var_754:Boolean;
      
      private var var_1865:BitmapData;
      
      private const const_1975:int = 13421772;
      
      private const const_1974:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_652 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1864 = Math.max(this.var_1864,_loc6_.colorLayerIndex);
            }
         }
         this.var_429 = this.var_68.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_652 != null && !this.var_1866)
            {
               _loc3_ = this.var_652;
            }
            else
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1975;
         }
         else
         {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_429.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_429.isFigureReady(_loc1_))
         {
            this.var_429.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_333[_loc6_] + "_" + FigureData.const_131;
               _loc4_ = this.var_429.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               if(this.var_1866 && _loc2_.colorLayerIndex > 0)
               {
