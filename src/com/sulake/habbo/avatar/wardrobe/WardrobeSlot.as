package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_347 = this.var_17.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_569 && this.var_1342)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_86,this.var_873,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_35,parseInt(FigureData.const_710));
         ...
         if(this.var_347)
         {
            if(this.var_347.bitmap)
            {
               this.var_347.bitmap.dispose();
            }
            this.var_347.bitmap = new BitmapData(this.var_347.width,this.var_347.height,true,0);
            _loc7_ = (this.var_347.width - _loc1_.width) / 2;
            _loc8_ = (this.var_347.height - _loc1_.height) / 2;
            this.var_347.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_569 = this._controller.figureData.getFigureString();
               this.var_873 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1667,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_569)
               {
                  this._controller.loadAvatarInEditor(this.var_569,this.var_873,this._controller.clubMemberLevel);
               }
         }
      }
