package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class MarketPlaceCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener, IMarketPlaceVisualization
   {
      ...
      private var var_1257:MarketPlaceOfferData;
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_454)
         {
            this.var_454.removeEventListener(TimerEvent.TIMER,this.onPopulationTimer);
            this.var_454 = null;
         }
      }
      ...
      private function selectSearchCategory(param1:String) : void
      {
         ...
         switch(param1)
         {
            ...
            case "search_by_activity":
               _loc5_ = "marketplace_search_simple";
               this.var_658 = [3,4,5,6];
               break;
            case "search_advanced":
               _loc5_ = "marketplace_search_advanced";
               this.var_658 = [1,2,3,4,5,6];
               break;
            default:
               return;
         }
         ...
         if(_loc7_ != null)
         {
            _loc7_.populate(this.getSortKeys(this.var_658));
            _loc7_.selection = 0;
         }
      }
      ...
      private function updateList() : void
      {
         ...
         if(!this.var_539)
         {
            ...
            this.var_539 = _loc4_.buildFromXML(_loc3_) as IWindowContainer;
         }
         if(!this.var_539)
         {
            return;
         ...
         this.updateStatusDisplay(this.const_1684,_loc2_.length);
         if(!this.var_454)
         {
            this.var_454 = new Timer(25);
            this.var_454.addEventListener(TimerEvent.TIMER,this.onPopulationTimer);
         }
         this.var_1003 = 0;
         this.populateList();
         this.var_454.start();
      ...
      private function populateList() : Boolean
      {
         ...
         while(_loc1_ < 5)
         {
            if(this.var_1003 >= this._offers.length)
            {
               return true;
            }
            this.addListItem(this._offers.getWithIndex(this.var_1003));
            ++this.var_1003;
            _loc1_++;
         }
         return false;
      ...
      private function addListItem(param1:MarketPlaceOfferData) : void
      {
         ...
         var _loc10_:* = null;
         var _loc11_:* = null;
         if(!param1 || !this._itemList || !this.var_539 || !this.marketPlace.localization)
         {
            return;
         }
         var _loc2_:IWindowContainer = this.var_539.clone() as IWindowContainer;
      ...
      private function doSearch() : void
      {
         ...
         if(_loc6_ && _loc6_.selection >= 0 && _loc6_.selection < this.var_658.length)
         {
            _loc4_ = this.var_658[_loc6_.selection];
         }
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "back_button":
                  ...
                  break;
               case "buy_button":
                  this.marketPlace.buyOffer(this.var_1257.offerId);
            }
            return;
         }
         if(param1.type == WindowEvent.const_60)
